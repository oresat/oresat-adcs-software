...
def run(show_plots, orbitCase, init_position, init_velocity, simulation_time_s, init_epoch):
    """
    ...
    if orbitCase == 'elliptical':
        plt.plot(timeAxis, [magModule.envMinReach/1000.]*len(rData), color='#007700', dashes=[5, 5, 5, 5])
        plt.plot(timeAxis, [magModule.envMaxReach / 1000.] * len(rData),
...
if __name__ == "__main__":
    ...
    print("ORESAT0.5 Orbit is not very eliptical so ignore the first plot")
    
    t_0 = (2024, 9, 4, 12, 0, 0)

    run(
        True,          # show_plots
        'elliptical',  # orbit Case (circular, elliptical)o
        init_position, # initial position meters
        init_velocity, # inital velocity meters/s
        simulation_time_s = 5400, # simulation time seconds
        init_epoch = '2024 September 4, 12:0:0.0 (UTC)'
    )
