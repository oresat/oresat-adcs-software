...
from adcs_lib import manager, simulator, quaternion
import copy
import numpy as np
...
class ADCSManager():
    ...
    man = manager.ManagerDaemonInterface(T_CTRL, T_CTRL, sim.model)
...
#        os.remove(PID_FILE)
    ...
    def run(self):
        ...
        while(self._running):
            ...
            #raw_sensor_data = self.sim.output(noisy=True)
            ...
            if current_state == State.SLEEP.value or current_state == State.FAILED.value:
                cmds = self.man.propagate(T_SLEEP, [current_state, np.zeros(3), np.zeros(3), np.zeros(3)], raw_sensor_data)
                sim_output = self.sim.propagate(T_SLEEP, cmds[:2])
                #time.sleep(0.5)
                #continue
                ...
                cmds = self.man.propagate(T_CTRL, [State.POINT.value, np.zeros(3), np.array([0, 0, -1]), np.zeros(3)], raw_sensor_data)
                sim_output = self.sim.propagate(T_CTRL, cmds[:2])
            elif current_state == State.BBQ.value:
                cmds = self.man.propagate(T_CTRL, [State.BBQ.value, np.zeros(3), np.array([0, 0, -1]), np.zeros(3)], raw_sensor_data)
                sim_output = self.sim.propagate(T_CTRL, cmds[:2])
            ...
            # for debugging
            self.true_states.append(np.block([x, v, q, w, W, B, S, self.man.filter.PosFilter.model.satellite.magnetorquers.power(mag_commands), *mag_commands]))
            self.kalman_states.append(np.block([*cmds[-1], *quaternion.error_quat(q, cmds[-1][2])[1:]*0.5]))
            if current_state == State.DETUMBLE.value and np.linalg.norm(w) < 0.01:
                print('done detumbling')
                self._dbus_server._sm.change_state(State.BBQ.value)
                self.man.filter.PosFilter.model.clock.absolute = 0
            if current_state == State.BBQ.value and self.man.rw_controller.converged:
                print('done bbqing')
                self.man.filter.PosFilter.model.clock.absolute = 0
                self._dbus_server._sm.change_state(State.SLEEP.value)
            if current_state == State.SLEEP.value and self.man.filter.PosFilter.model.clock.absolute > 1000:
                self.quit()
    ...
    def quit(self):
        ...
        np.savetxt('true_states.csv', self.true_states, delimiter=',', comments='',
            header='x_x,x_y,x_z,v_x,v_y,v_z,q0,q1,q2,q3,w_x,w_y,w_z,W1,W2,W3,W4,B_x,B_y,B_z,S_x,S_y,S_z,P,I_x,I_y,I_z')
