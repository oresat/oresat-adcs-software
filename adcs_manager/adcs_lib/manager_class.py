...
class ManagerDaemonInterface():
    ...
    def __init__(self, gyro_step_size, gps_step_size, truth_model):
        ...
        self.rw_controller        = controller.ReactionWheelsController(self.filter.PosFilter.model, 0.707, 0.15)
    ...
    def mission_input(self, mission_data):
        ...
        #elif mission_data[0] == State.BBQ.value:
        #    self.mag_cmd = self.mag_controller.desaturate2(model_state, np.zeros(4))
        #    self.rw_cmd  = self.rw_controller.induce_bbq_roll_momentum(model_state)
        elif mission_data[0] == State.POWERDOWN.value:
            self.mag_cmd = np.zeros(3)
            null = np.array([-0.5, 0.5, -0.5, 0.5])
            mult = self.mag_controller.k_bbq * 100
            self.rw_cmd = - np.dot(model_state[4], null) * null
            if np.linalg.norm(self.rw_cmd) < 0.001:
                self.rw_cmd = - model_state[4]
                if np.linalg.norm(model_state[4]) < 0.1:
                    self.rw_cmd = np.zeros(4)
                else:
                    self.rw_cmd *= mult
            #self.rw_cmd = np.zeros(4)
