...
\begin{itemize}
	...
	\item Target coordinates (latitude, longitude, altitude) for ground target tracking mode
...
The control system is configured through a set of parameters that determine which actuators are active and how control laws are applied:

\begin{itemize}[itemsep=6pt]
	\item \texttt{control\_mode}: Defines actuator usage and control laws. Possible modes:
	\begin{itemize}
		\item DETUMBLE: Uses magnetorquers for long-duration detumbling.
		\item RW\_POINTING: Uses reaction wheels for fine-pointing. Guidance for target orientation is defined separately.
		\item MTB\_POINTING: Uses magnetorquers for coarse-pointing over long periods of time. Guidance for target orientation is defined separately.
		\item THERMAL\_DETUMBLE: Initiates long-duration detumble, which then transitions to reorientation with reaction wheels to reorient satellite perpendicular to sun vector, and then spin up about the z-axis for passive thermal-stabilization.
	\end{itemize}
	
	\item \texttt{pointing\_reference}: defines boresight-reference or pointing-reference axis of the spacecraft for guidance calculations. Possible modes:
	\begin{itemize}
		\item HELICAL: Uses the helical antenna (OreSat +Z axis) as the primary pointing reference
		\item CFC: Uses the Cirrus Flux Cam (OreSat1's -Z axis) as the primary pointing reference
		\item ST: Uses the Star Tracker as the primary pointing reference. Added as a potential reference, not actually used in missions.
	\end{itemize}
...
\section{Coordinate Frames and Attitude Representations}
...
Coordinate frames provide a structured way to represent position, velocity, and attitude, and to transform these quantities between different reference systems used in the ADCS.

\begin{itemize}
	\item Body frame (B): A coordinate system fixed to the spacecraft, with origin at the center of mass and axes aligned with the spacecraft-defined $+x$, $+y$, and $+z$ body axes as specified by the mechanical and systems design. All onboard quantities such as angular velocity, control torque, and actuator commands are expressed in this frame.
	\item Inertial frame (ECI): A non-rotating coordinate system centered at the Earth's center of mass, which remains fixed relative to the starfield. This frame is used as the reference for attitude determination and orbital dynamics. The star tracker's output is defined in this frame.
	\item Earth-fixed frame (ECEF): A coordinate system centered at the Earth's center of mass that rotates with the Earth. In this frame, locations on the Earth's surface remain fixed, making it suitable for representing ground stations and geographic targets. % This frame is used for guidance modes involving all ground targets and attitudes relative to the Earth (such as the ram-facing guidance mode).
\end{itemize}
