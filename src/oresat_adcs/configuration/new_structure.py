...
class Wall():
    ...
    def srp_force_torque(self, SRP, S):
    ...
    def drag_force_torque(self, drag_pressure, v):
        '''Calculates solar radiation pressure torque on this wall.
        Assumes there is no diffuse reflection.
...
class Satellite():
    ...
    def srp_force_torque(self, SRP, S):
        ...
        Parameters
        ----------
        SRP : float : Solar radiation pressure.
        S : numpy.ndarray : Sun vector in body coordinates.

        Returns
        -------
        numpy.ndarray : Force (N) and torque (N m) on satellite.
        '''
        F_and_T = sum([wall.srp_force_torque(SRP, S) for wall in self.walls])
        return F_and_T
    ...
    def drag_force_torque(self, drag_pressure, v):
        ...
        Parameters
        ----------
        drag_pressure : float : the normallized drag pressure
        v: numpy.ndarray : the velocity (check which frame)

        Returns
        -------
        numpy.ndarray : Force (N) and torque (N m) on satellite.
        '''
        F_and_T = sum([wall.drag_force_torque(drag_pressure, v) for wall in self.walls])
        return F_and_T

    def load_json(config_file_path, max_T, torque_limited=True):
...
class ReducedSatellite():
    ...
    These classes are so different it does not warrant inheritance'''

    def __init__(self, mass, area, drag_coefficient, absorption):
        self.mass = mass
        self.area = area
        self.drag_coeffcient = drag_coefficient
        self.absorption = absorption
        self.reflection = 1 - absorption


    def srp_force(self, solar_radiation_pressure, sun_vector):
        '''Calculates the srp force from parameters

        Parameters:
            srp : float : solar radiation pressure
            sun_vector : numpy.ndarray : normalized vector for sun direction
        
        Returns:
            numpy.ndarray : solar radiation force vector
        '''
        return - (solar_radiaiton_pressure * (2*self.reflection + self.absorption) * sun_vector)

    def drag_force(self, rho, velocity_vector):
        '''Calculates the drag force from parameters

        Parameters:
            rho : float : density of air
            velocity_vector : numpy.ndarray : velecity vector of air, NOT normalized

        Returns:
            np.ndarray : the drag force and torque acting on the satellite
        '''
        # density of air, check that the magnitude of the vector works
        return (- 0.5 * rho * self.area * velocity_vector * v_velocity_vector)

    def gravity_force(self, gravity_vector):
        '''Calculates the gravity force vector

        Parameters:
            gravity_vector : numpy.ndarray : the vector that gravity acts on, NOT normalized

        Returns:
            np.ndarray : the gravity force vector
        '''
        return self.mass * gravity_vector
