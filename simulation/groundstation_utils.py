...
def kepler_values(r0_vec: np.ndarray, v0_vec: np.ndarray, dt: int) -> np.ndarray:
    ...
    alpha = (
        -2 * epsilon / mu_earth
    )  # alpha should always be > 0 for an elliptical orbit
...
def time_to_overpass(
    ...
    maximum_window = (
        3600  # large value to ensure exit boundary of overpass window is found
    )
    ...
    while time_offset < t_end:
        ...
        for dt in range(lower_bound, upper_bound, small_dt):
            if distance_to_target_eci(dt) <= max_distance:
                ...
                logger.info(
                    f"Overpass window entry predicted to occur in {window_start} seconds"
                )
                break

        # third step: find window exit time
        for dt in range(
            window_start, window_start + maximum_window, exit_check_increment
        ):
            if distance_to_target_eci(dt) > max_distance:
                window_exit = dt  # window exit at this time
                logger.info(
                    f"Overpass window exit predicted to occur in {window_exit} seconds"
                )
                break
...
def find_nearest_ground_station(
    ...
    for station in station_list:
        ...
        if (start is not None) and (
            (next_overpass is None) or (start < next_overpass[0])
        ):
    ...
    if chosen_station:
        ...
        logger.info(
            f"Overpass window entry predicted to occur in {next_overpass[0]} seconds"
        )
        logger.info(
            f"Overpass window exit predicted to occur in {next_overpass[1]} seconds"
        )
